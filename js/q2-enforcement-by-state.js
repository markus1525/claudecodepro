...
(function() {
  ...
  function addLegend(svg, data, width, height, sizeScale, isMobile) {
    ...
    const sizeExamples = isMobile ? [
      { value: highState.totalFines, label: 'High', radius: 20, color: colorScale(highState.code) },
      { value: mediumState.totalFines, label: 'Med', radius: 13, color: colorScale(mediumState.code) },
